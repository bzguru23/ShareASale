...
namespace ShareASale\TrackAffiliates\Model\Import;

use Magento\ImportExport\Model\Import\ErrorProcessing\ProcessingErrorAggregatorInterface;
use Magento\Framework\App\ResourceConnection;

class Affiliate extends \Magento\ImportExport\Model\Import\Entity\AbstractEntity
{

    const ID = 'entity_id';
    const AFFILIATEID = 'affiliate_id';
  ...
  const TITLE = 'affiliate_import_code';  
  const TABLE_Entity = 'affiliate';
    ...
    while ($bunch = $this->_dataSourceModel->getNextBunch()) {
        ...
        foreach ($bunch as $rowNum => $rowData) {
            ...
            $entityList[$rowTtile][] = [
              ...
              self::ID => $rowData[self::ID],
              self::AFFILIATEID => $rowData[self::AFFILIATEID],
              self::FIRSTNAME => $rowData[self::FIRSTNAME],
              self::MIDDLENAME => $rowData[self::MIDDLENAME],
    ...
    if ($table && $listTitle) {
            try {
                $this->countItemsDeleted += $this->_connection->delete(
                    ...
                    $this->_connection->quoteInto('affiliate_import_code IN (?)', $listTitle)
                );
                return true;
            } catch (\Exception $e) {
                return false;
